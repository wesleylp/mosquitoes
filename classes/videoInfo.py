...
class videoInfo(object):
    ...
    def __init__(self, video_file):
        ...
        if os.path.isfile(video_file):
            ...
            if self.isVideo():
                self._codec = ffoutput['streams'][self._idxVideoInfo][
                    'codec_name']
                self._codecLong = ffoutput['streams'][self._idxVideoInfo][
                    'codec_long_name']
                self._width = ffoutput['streams'][self._idxVideoInfo]['width']
                self._height = ffoutput['streams'][self._idxVideoInfo][
                    'height']
                self._widthHeight = [self._width, self._height]
                self._sampleAspectRatio = ffoutput['streams'][
                    self._idxVideoInfo]['sample_aspect_ratio']
                self._displayAspectRatio = ffoutput['streams'][
                    self._idxVideoInfo]['display_aspect_ratio']
                self._pixelFormat = ffoutput['streams'][self._idxVideoInfo][
                    'pix_fmt']
                self._frameRate = ffoutput['streams'][self._idxVideoInfo][
                    'r_frame_rate']
                self._framesPerSecond = int(
                    self._frameRate[:self._frameRate.index('/')])
                self._durationTS = ffoutput['streams'][self._idxVideoInfo][
                    'duration_ts']
                self._duration = ffoutput['streams'][self._idxVideoInfo][
                    'duration']
                self._bitRate = ffoutput['streams'][self._idxVideoInfo][
                    'bit_rate']
                self._numberOfFrames = ffoutput['streams'][self._idxVideoInfo][
                    'nb_frames']
        else:
    ...
    def printAllInformation(self):
        ...
        print('File extension: ' + str(self._format) + ' (' +
              str(self._formatLong) + ')')
